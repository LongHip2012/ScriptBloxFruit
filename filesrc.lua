...
spawn(function() 
    ...
    wait(1.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>BubbleGum Hub Chào Mng<Color=/>"):Display()
    wait(.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Yellow>Hãy sử dụng Fluxus để có trải nghiệm tốt nhất!<Color=/>"):Display()
    wait(.24)
    ...
    wait(.29)
    require(game.ReplicatedStorage.Notification).new("<Color=Yellow>Đăng Ký Kênh YouTube FTGxHoangLong để xem video mới nhất nhé<Color=/>"):Display()
    wait(.36)
end)

local OrionLib = loadstring(game:HttpGet(('https://raw.githubusercontent.com/shlexware/Orion/main/source')))()
local Window = OrionLib:MakeWindow({Name = "BubbleGum Hub", HidePremium = false, IntroText = "BubbleGumHub", SaveConfig = true, ConfigFolder = "NightHub"})
...
OrionLib:MakeNotification({
    Name = "BubbleGum Hub",
    Content = "Đăng Ký Kênh YouTube  FTGxHoangLong Nhé💕",
    Image = "rbxassetid://8192162908",
    Time = 10
})
...
W:AddLabel("FTGxAether")
W:AddLabel("Discord Genshin Hub | Community")
local Section = W:AddSection({
    Name = "Status"
})
...
pcall(function()
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                OrionLib:MakeNotification({
                    Name = "BubbleGum Hub(Noitification)",
